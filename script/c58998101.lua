...
function s.initial_effect(c) 
	...
	c:RegisterEffect(e2)
	--Apply Queen Ambrosé's effect, then add 1 "Horticopia" Ritual Monster and 1 "Horticopia Paradise Banquet" to hand
...
function s.fldflt(c)
	return c:IsAbleToHand() and c:IsSetCard(0x888) and (c:IsRitualMonster() or c:IsRitualSpell())
end
function s.fldflt(c,tp)
	return c:IsCode(58998122) and c:IsAbleToHand()
		and Duel.IsExistingMatchingCard(s.thfilter2,tp,LOCATION_DECK,0,1,c)
end
function s.fldflt2(c)
	return c:IsType(TYPE_RITUAL) and c:IsMonster() and c:IsSetCard(0x888) and c:IsAbleToHand()
end
function s.fldop(e,tp,eg,ep,ev,re,r,rp)
	...
	Duel.BreakEffect()
	--Add 1 "Horticopia" Ritual Monster and 1 "Horticopia Paradise Banquet" to hand
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g1=Duel.SelectMatchingCard(tp,s.fldflt,tp,LOCATION_DECK,0,1,1,nil,tp)
	if #g1>0 then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
		local g2=Duel.SelectMatchingCard(tp,s.fldflt2,tp,LOCATION_DECK,0,1,1,nil,tp)
		g1:Merge(g2)
